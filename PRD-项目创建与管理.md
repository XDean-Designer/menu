...
- **「全部」Tab 列表结构**（自上而下）：  
  ...
  3. 底部折叠条「**已隐藏 · 仅顾客端不可见**」（无数量；块底 `#EEEEEE`，与展开区 `#F7F7F7` 区分；**分区线在其上方**）；展开后可查看/操作隐藏条目  
...
- **分区标题与分区线**：列表分区自上而下为「在售 → 已下架 → 已隐藏」，两个分区标题**各带副标题点出业务差异** —— **`已下架 · 停售`**（13px/400/`--text-sec`/居中，`.catalog-offsale-head`）与 **`已隐藏 · 仅顾客端不可见`**（15px/400/`--text`/左对齐 + 折叠箭头，`.catalog-hidden-toggle__label`）；**「仅」字不可省**（下架同样对顾客端不可见，去掉「仅」后副标题反而与「已下架」同义）。**分区线一律画在标题上方**（`16px 0` / `::before top:0`，左右各内缩 16、`#EBEBEB`），两个分区方向必须一致；**标题与它自己的内容之间不得有线**。在售末行必须**让位**给分区线（`#projProjectTable .table-row-swipe:has(+ .catalog-offsale-block) .table-row::after{display:none}` —— 须带 ID 前缀才生效，否则被上方的 `display:block` 覆盖并与分区线叠成重线）。
...
24. **Given** 「全部」列表有下架与隐藏条目，**When** 浏览，**Then** 无状态列；可见居中「已下架 · 停售」分区标题与「已隐藏 · 仅顾客端不可见」折叠条（无数量），**两条分区线各自在其标题上方**（左右内缩 16），且**在售末行不再重复画线**（该区段共 2 条线，不得出现 3 条）。  
