...
/* 「保存」按钮里的图标：**存入托盘**（arrow → line）线描稿，v1.31 起替换原「实心软盘」。
   改为线描的三条理由：① 本模块 19 个图标里 17 个是线描（Stratis 族），实心软盘是唯一的普通控件类实心图标；
   ② 原图标 13.5px 框里塞了 3 段子路径（外框 + 快门 + 标签块），16px 下过密，还要靠 `stroke-width="0.2"` 补线；
   ③ 按钮 v1.31 改 tinted 淡底后，那块实心红方块成了按钮上最重的东西，比 16px 的「保存」二字还抢眼。
   几何：viewBox 24 / stroke 2.3 → 渲染 16px 时线宽约 1.53px（与图标族 1.56px 同档）；
   墨迹归一化到 13.0 × 13.27px（≈ 原软盘的 13.5px），换图标不改变视觉重量。
   `fill="none"` + `stroke="currentColor"` 全由本 SVG 自带 —— base.css 里那条 `path { fill: currentColor }` 已随之删除。 */
const PROJ_SAVE_ICON = '<svg class="catalog-inline-save__icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.3" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3.4v11.2"/><path d="M6.6 9.6l5.4 5.4 5.4-5.4"/><path d="M3.4 21h17.2"/></svg>';
